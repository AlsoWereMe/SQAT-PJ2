...
def delete_random_bytes(s: str) -> str:
    ...
    if not bytes_arr:
        return s
    # 仅允许删除长度 <= 当前字节数的N，且确保删除后不全部清空
    possible_n = [n for n in [1, 2, 4] if n < len(bytes_arr)]  # 修改条件为 n < len(bytes_arr)
    if not possible_n:
        return s
...
def sample2_aware_insert(s: str) -> str:
    """专门针对Sample2的插入策略"""
    # 55%概率在关键位置插入点号
...
def generate_edge_cases(_: str) -> str:
    """专门生成边界测试用例"""
    patterns = [
        "",          # 空输入
        ".",         # 仅分隔符
...
class Sample2Mutator(Mutator):
    """仅用于Sample2的专用变异器"""
    def __init__(self):
        super().__init__()
        # 重写变异器列表和权重
        self.mutators = [
            sample2_aware_insert,  # 新增针对性插入
            flip_random_bits,
            arithmetic_random_bytes,
            lambda s: delete_random_bytes(s).replace("..", "."),  # 防止连续删除导致多分隔符
            havoc_random_replace
        ]
        self.weights = [0.5, 3, 2, 2,  1]

    def _enhanced_insert(self, s: str) -> str:
        """样本2专用插入：50%概率插入点号"""
        if random.random() < 0.5 and len(s) > 0:
            # 优先在已有点号后插入
            dot_pos = s.find('.')
            if dot_pos != -1:
                return s[:dot_pos+1] + '.' + s[dot_pos+1:]
        return insert_random_character(s)
    
    def _safe_delete(self, s: str) -> str:
        """防止过度删除导致结构破坏"""
        modified = delete_random_bytes(s)
        # 确保删除后仍然包含点号
        if '.' not in modified and '.' in s:
            return s.replace(random.choice([c for c in s if c != '.']), '', 1)
        return modified
    
    def mutate(self, inp: Any) -> Any:
        # 1%概率直接生成边界用例
        if random.random() < 0.0001:
            return generate_edge_cases(inp)
        return random.choices(self.mutators, weights=self.weights, k=1)[0](inp)
